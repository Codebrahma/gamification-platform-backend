...
describe('## Weather APIs', () => {
	describe('# GET /getWeatherByCityName', () => {
		...
		it('should return weather for the given city', async () => {
			...
			getWeatherByCityNameStub = sinon.stub(weatherService, 'getWeatherByCityName').callsFake(async () => Promise.resolve(apiResponse));
		...
		it('should return internal server error', async () => {
			// mock getWeatherByCityName
			getWeatherByCityNameStub = sinon.stub(
				weatherService,
				'getWeatherByCityName',
			).callsFake(async () => Promise.reject(new Error(`Failed to fetch weather for ${cityName}`)));
